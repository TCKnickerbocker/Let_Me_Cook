    ...
    if (auth_header) {
      ...
      if (foodName) {
        ...
        const result = await client
          .db("LetMeCookDB")
          .collection("foods")
          .find({ userId: userId, name: foodQuery })
          .toArray();
        client.close();
        return {
          status: 201,
...
app.http("getFood", {
  ...
  handler: async (request, context) => {
    ...
    return {
      status: 403,
      jsonBody: {
        error: "Authorization failed for obtaining food information",
      },
    };
  },
});
...
app.http("insertFood", {
  ...
  handler: async (request, context) => {
    ...
    if (auth_header) {
      ...
      await client
        ...
        .updateOne(
          ...
          { $push: { foods: { _id: result.insertedId } } }
        );
      const userInfo = await client
        .db("LetMeCookDB")
        .collection("users")
        .findOne({ name: userName, userId: userId });
    ...
    return {
      status: 403,
      jsonBody: {
        error: "Authorization failed for entering food into user's pantry",
      },
    };
  },
});
  ...
  handler: async (request, context) => {
    ...
    if (auth_header) {
      ...
      const userResult = await client
        ...
        .updateOne(
          { userId: userId },
          { $push: { recipes: { _id: recipeResult.insertedId } } }
        );
      ...
      if (auth_header) {
        ...
        if (body) {
          ...
          return {
            status: 403,
            jsonBody: {
              error: "Authorization failed for editing food with id: " + _id,
            },
          };
        }
        return {
          status: 405,
          jsonBody: {
            error: "Empty content sent to edit food with id: " + _id,
          },
        };
      }
      return {
  ...
  handler: async (request, context) => {
    ...
    if (ObjectId.isValid(_id)) {
      ...
      if (auth_header) {
        ...
        // Upload buffer
        const uploadResult = await blobClient.uploadData(
    ...
    if (ObjectId.isValid(_id)) {
      ...
      if (auth_header) {
        ...
        return {
          status: 403,
          jsonBody: {
            error: "Authorization failed for editing recipe with id: " + _id,
          },
        };
      }
      return {
    ...
    if (ObjectId.isValid(_id)) {
      ...
      if (auth_header) {
        ...
        const resultOne = await client
          ...
          .updateOne(
            { userId: userId },
            { $pull: { foods: { _id: new ObjectId(_id) } } }
          );
      ...
      return {
        status: 403,
        jsonBody: {
          error: "Authorization failed for removing food with id: " + _id,
        },
      };
    }
    return {
    ...
    if (ObjectId.isValid(_id)) {
      ...
      if (auth_header) {
        ...
        const resultOne = await client
          ...
          .updateOne(
            { userId: userId },
            { $pull: { recipes: { _id: new ObjectId(_id) } } }
          );
        const resultTwo = await client
          .db("LetMeCookDB")
          .collection("users")
          .updateOne({ userId: userId }, { $pull: { recipeQueue: _id } });
      ...
      return {
        status: 403,
        jsonBody: {
          error: "Authorization failed for removing recipe with id: " + _id,
        },
      };
    }
    return {
  ...
  handler: async (request, context) => {
    ...
    if (auth_header) {
      ...
      const result = await client
        ...
        .updateOne(
          { userId: userId },
          { $push: { recipeQueue: { recipe: recipe } } }
        );
      client.close();
      if (result.matchedCount > 0) {
        return {
  ...
  handler: async (request, context) => {
    ...
    if (auth_header) {
      ...
      const result = await client
        .db("LetMeCookDB")
        .collection("users")
        .updateOne({ userId: userId }, { $set: { recipeQueue: recipes } });
      client.close();
      return {
        status: 201,
        jsonBody: { recipes: recipes },
...
app.http("expiringFoods", {
  ...
  handler: async (request, context) => {
    ...
    if (auth_header) {
      ...
      context.log("\n\n\n");
      const client = await mongoClient.connect(process.env.AZURE_MONGO_DB);
      const expiringFoods = await client
        .db("LetMeCookDB")
        .collection("foods")
        .find({ userId: userId })
    ...
    return {
      status: 403,
      jsonBody: {
        error: "Authorization failed for fetching expiring food info",
      },
    };
  },
});
...
app.http("lowestFoods", {
  ...
  handler: async (request, context) => {
    ...
    return {
      status: 403,
      jsonBody: {
        error: "Authorization failed for fetching lowest quantity food info",
      },
    };
  },
});
  ...
  handler: async (request, context) => {
    ...
    if (auth_header) {
      ...
      const foods = client
        .db("LetMeCookDB")
        .collection("foods")
        .find({ userId: userId }, { $orderby: { timestamp: -1 } })
        .toArray();
      client.close();
      return {
        status: 201,
    ...
    return {
      status: 405,
      jsonBody: {
        error: "Unable to authorize user to obtain foods by timestamp",
      },
    };
  },
});
    ...
    if (auth_header) {
      ...
      if (searchTerm) {
        const recipes = await client
          ...
          .find(
            { userId: userId },
            { $elemMatch: { ingredients: { name: /.*searchTerm.*/ } } }
          )
          .toArray();
        client.close();
        if (recipes) {
