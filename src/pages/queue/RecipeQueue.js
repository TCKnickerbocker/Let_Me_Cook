...
export default function RecipeQueue() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Will happen when the user has nothing in the recipe queue
      if (body != {}) {
  ...
  return (
    <>
      <AddRecipeQueuePopup addRecipe={addRecipe} />
      <div className={styles.queueList} align="left">
          ...
          recipeIds.map((recpId, i) => (
            <div
              ...
              className={`${styles.grow} ${
                recpId === draggingRecpIndex ? "dragging" : ""
              }`}
              draggable="true"
        ...
        {!recipeIds.length ? (
          <div className={Layout.center}>
            <h2>No Recipes in queue</h2>
            <p>you can add saved recipes to your queue here </p>
          </div>
        ) : (
          <div></div>
        )}
